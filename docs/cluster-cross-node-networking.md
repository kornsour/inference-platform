# Cross-node pod networking — why it matters & current status
...
| Capability you need | Needs cross-node pod networking? | Why |
|---|---|---|
...
!!! note "Verdict"
    ...
    inference-aware signal — **yes, you need working cross-node pod networking.** Not for
    the GPUs themselves, but for the **observability + traffic plane** that makes two boxes
    ...
    **You are not blocked, though.** The single-GPU **Option A** mock already proves the
...
specialized networking layered on top that you deliberately don't need at home:
...
> Interview framing: *at small scale you need pod-to-pod just for the control/observability
> plane; at production scale it becomes a first-class performance surface — the difference
...
|---|---|---|
| Server host → laptop **node** `192.168.18.142` | :material-check: 0% loss, `ttl=128` | `ttl=128` ⇒ the **Windows host** answered, not Linux — confirms mirrored mode |
...
- **TCP inbound to the laptop already works.** `kubectl exec` into a laptop pod succeeded,
  ...
  **specifically inbound UDP 8472 on the laptop** — a tractable, targeted fix.
...
(6443/TCP and 10250/TCP are already working — the agent joined and `kubectl exec`
...
- **Nothing** → mirrored-mode isn't delivering inbound UDP at all. Try `firewall=false` in
  the laptop's `.wslconfig` + `wsl --shutdown` as a test; if that's still dead, treat
