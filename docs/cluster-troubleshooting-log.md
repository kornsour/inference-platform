...
traffic. This is the full journey: the wrong turns, the experiments that reframed the
problem, the root cause, and the fix. It's here partly as a runbook and partly because
*how* you corner a bug like this is the actual skill.
...
!!! success "TL;DR"
    ...
    **Fix:** run the metrics pods with `hostNetwork: true` and scrape them on the node IP
    over TCP — sidestepping the overlay for the observability plane that the autoscaling
    demo actually needs. Result: both GPUs reporting into one Prometheus.

---

## 1. What we were building (and the stakes)

The capstone's headline is **real multi-replica GPU autoscaling** — KEDA scaling vLLM
across two *physical* GPUs (a desktop RTX 3060 Ti + a laptop RTX 4070) on an
inference-aware signal. That needs Prometheus to scrape inference + GPU metrics from pods
...
```bash
cat /sys/class/net/flannel.1/statistics/tx_packets   # climbing — we send fine
cat /sys/class/net/flannel.1/statistics/rx_packets   # 0 — we have NEVER received one
```
...
The most popular WSL2 answer online. We had already, over prior sessions, added the
...
This session we confirmed it cold from inside both VMs:
...
won't route a foreign pod subnet into its Linux VM. A brief experiment confirmed it;
reverted. VXLAN is in fact the *correct* backend for this topology. Killed.
...
`CHECKSUM_PARTIAL` expecting hardware to finish it; the virtual path never does). So we
...
No effect on `RX`. And on closer reading, that `bad cksum` line was a **tcpdump artifact**
of capturing a locally-generated packet *before* offload — plus the VXLAN device uses a
zero outer checksum by default anyway, which receivers don't validate. Reverted both
changes. Killed.
...
So we changed the setup: stand up SSH **directly into both WSL instances** (port 2222,
...
**reach the GPU/inference metrics on both nodes**. Those are TCP, and TCP to a
...
The `hostNetwork` fix unblocks the capstone, but if you want genuine cross-node pod-to-pod
...
- **Tailscale** (`k3s --vpn-auth`) — `tailscaled` is a userspace daemon (its socket *is*
  ...
  TCP already works here, this connects even in the worst case. The recommended real-overlay
  path.
...
The transferable habits, separated from the WSL specifics:
...
- **Verify the measurement, not just the result.** The breakthrough came from noticing the
  *tool* (tcpdump without a bound socket) was lying — the bug was partly in how we were
  looking.
...
- **Know when to fix vs. sidestep.** The goal was a working autoscaling signal, not a
  philosophically pure overlay. `hostNetwork` shipped the outcome today; Tailscale remains
  available for the overlay itself.
