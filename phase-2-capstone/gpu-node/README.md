...
**8 GB** consumer GPUs (an RTX 3060 Ti + an RTX 4070 Laptop), joined as a k3s
cluster, serve real vLLM **and** demonstrate genuine multi-replica GPU scale-out —
for **$0**, with the same cost discipline the platform role is testing for.
...
vLLM needs room for **weights + KV cache + overhead**. From the actual runs
([results](real-gpu-results.md)):

- **1.5B FP16** (~3 GB) — comfortable, ~100k tokens of KV cache; *compute-bound* on
  an 8 GB card (never trips the KV-cache scaling signal).
- **3B FP16** (~6 GB) — only fits with `--enforce-eager --max-model-len 2048`;
  *KV-bound* (~12k tokens of KV cache), which is what actually exercises the
  autoscaler's queue-depth / KV-cache signal.
...
external LB spreads traffic across them. Full setup in the [runbook](diy-cluster.md);
...
Service at it ([`external-vllm.yaml`](external-vllm.yaml)) — real numbers, but the pod
isn't Kubernetes-scheduled. The in-cluster two-GPU build above is the real artifact.

## Bottom line

Two 8 GB consumer GPUs on the LAN delivered the full serving story — real vLLM
numbers **and** multi-replica scale-out — at **$0**. Renting a multi-GPU cloud node is
only worth it for what two 8 GB cards genuinely can't do: a single model too large for
