...
- [x] **Autoscale with KEDA** driven by TTFT p95 or KV-cache utilization from Prometheus, not CPU. Validated locally from 1 to 5 replicas, now live on the GPU cluster as a ScaledObject autoscaling vLLM on queue-depth plus KV-cache (HPA reading `0/3, 0/700m`). The default scaling path is the custom Go external scaler's composite trigger ([`keda-inference-scaler/deploy/scaledobject-external.yaml`](keda-inference-scaler/deploy/scaledobject-external.yaml)); the two-trigger built-in-scaler version ([`gpu-node/keda-scaledobject-gpu.yaml`](gpu-node/keda-scaledobject-gpu.yaml)) remains as a documented alternative. This is the platform's core capability.
- [ ] Add an **inference gateway** (Envoy AI Gateway) for token-aware rate limiting and model routing. Automated in the [bootstrap](../bootstrap/README.md) with route manifests written and now schema-linted in [CI](https://github.com/kornsour/inference-platform/blob/main/.github/workflows/ci.yml); live bring-up is pending because it needs helm and is the most overlay-sensitive layer.
